...
import com.ypitta.MultiZoneClimateControl.gateway_device.MqttClient.PerformanceDataMqttCallback;
import com.ypitta.MultiZoneClimateControl.gateway_device.common.ConfigUtil;

public class PerformanceSubscriberGenerator{
	...
	public PerformanceSubscriberGenerator(String zoneId) {
		this.zoneId = zoneId;
	...
	private void instantiate() {
		...
		this.aws_pub = new AWSIoTMqttClientSecure(this.endpoint, this.zoneId+"PerfData", this.certificateFile, this.privateKeyFile);
		try {
		...
		conn.ConnectMqttClientSetCallBack(new PerformanceDataMqttCallback(this.aws_pub, "update/performanceData/"+zoneId));
		
		this.sub_constrained = new MqttClientSubscriber(conn.getClient(), "system/performance/"+this.zoneId, Integer.valueOf(this.util.getValue("mqtt.cloud", "defaultQos")));
